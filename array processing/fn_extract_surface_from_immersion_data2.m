function [surf_x, surf_z, tfm_image] = fn_extract_surface_from_immersion_data2(exp_data, mesh, options)
...
lo_res_image = fn_fast_DAS3(exp_data, focal_law_lo_res, options.use_gpu_if_available);
lo_res_image = gather(lo_res_image);
tfm_image = zeros(size(mesh.x));
tfm_image(zi1:zi2, :) = lo_res_image(1:mesh_step_ratio:end, 1:mesh_step_ratio:end);
...
hi_res_image = fn_fast_DAS3(exp_data, focal_law_hi_res, options.use_gpu_if_available);
hi_res_image = gather(hi_res_image);
