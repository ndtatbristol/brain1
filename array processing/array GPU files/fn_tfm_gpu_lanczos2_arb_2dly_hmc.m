function result=fn_tfm_gpu_lanczos_arb_2dly_hmc(exp_data,TFM_focal_law,aFactor)
global SYSTEM_TYPES
...
lookup_time_tx= gpuArray(single(TFM_focal_law.lookup_time_tx));
lookup_time_rx= gpuArray(single(TFM_focal_law.lookup_time_rx));
...
bit_ver=mexext;
ptx_file=['gpu_tfm_lanczos2_arb_2dly_hmc_' bit_ver([end-1:end]) '.ptx'];

k = parallel.gpu.CUDAKernel(ptx_file, 'gpu_tfm_lanczos2_arb_2dly_hmc.cu');
...
[real_result imag_result]=feval(k,real_result,imag_result,n,combs,real_exp,img_exp,tx, rx, lookup_time_tx, lookup_time_rx,time, pixels, grid_x, grid_y, grid_z, lookup_amp_tx, lookup_amp_rx, tt_weight,aFactor);
