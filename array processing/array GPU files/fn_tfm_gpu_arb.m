function result=fn_tfm_gpu_arb(exp_data,TFM_focal_law)
global SYSTEM_TYPES
...
% result=GPUArray.ones(size(result),'single');
...
lookup_ind= int32(TFM_focal_law.lookup_ind);
lookup_ind=gpuArray(lookup_ind);
...
bit_ver=mexext;
ptx_file=['gpu_tfm_arb_' bit_ver([end-1:end]) '.ptx'];

k = parallel.gpu.CUDAKernel(ptx_file, 'gpu_tfm_arb.cu');
...
[real_result imag_result]=feval(k,real_result,imag_result,n,combs,real_exp,img_exp,tx, rx, lookup_ind, pixels, grid_x, grid_y, grid_z, lookup_amp, tt_weight);
%
result=real_result+imag_result.*i;
result=gather(result);
result=double(result);
%  imagesc(20.*log10(result./max(max(result))));
% caxis([-40 0])
% axis equal