function result=fn_tfm_gpu_lanczos2_arb(exp_data,TFM_focal_law,aFactor)
global SYSTEM_TYPES
...
bit_ver=mexext;
ptx_file=['gpu_tfm_lanczos2_arb_' bit_ver([end-1:end]) '.ptx'];

k = parallel.gpu.CUDAKernel(ptx_file, 'gpu_tfm_lanczos2_arb.cu');
k.ThreadBlockSize = TFM_focal_law.thread_size;%gpu_han.MaxThreadsPerBlock;
k.GridSize = ceil(pixs./k.ThreadBlockSize(1));
%k.SharedMemorySize=2000*4;
real_result=result;
imag_result=result;

[real_result imag_result]=feval(k,real_result,imag_result,n,combs,real_exp,img_exp,tx, rx, lookup_time,time, pixels, grid_x, grid_y, grid_z, lookup_amp,tt_weight,aFactor);
