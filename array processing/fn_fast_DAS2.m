function [result, varargout] = fn_fast_DAS2(exp_data, focal_law,varargin)
%USAGE
%   [result, [timings]] = fn_fast_DAS2(exp_data, focal_law,[use_gpu_if_available])
%SUMMARY
...
if use_gpu_if_available && (exist('gpuDeviceCount') == 2) && (gpuDeviceCount > 0)
    ...
    switch method
        case 'Same Tx and RX laws (nearest)'
            result = fn_tfm_gpu_arb(exp_data,focal_law);
        case 'Different Tx and RX laws, FMC data (nearest)'
            %Separate focal laws for Tx and Rx and FMC data
            result = fn_tfm_gpu_arb_2dly(exp_data,focal_law);
        case 'Different Tx and RX laws, HMC data (nearest)'
            result = fn_tfm_gpu_arb_2dly_hmc(exp_data,focal_law);
            
            %Linear interpolation methods
            
        case 'Same Tx and RX laws (linear)'
            result = fn_tfm_gpu_linear_arb(exp_data,focal_law);
