...
switch (precision)
    case 'single'
        ...
        focal_laws.pathDist=single(zeros(2,nelements,npixels,nRayPaths));
        focal_laws.matMode=zeros(2,nRayPaths);
        ...
        for i=1:nRayPaths
            ...
            switch (ii)
                case 1 %Contact
                    matMode=[1];
                    if (dimension_opt>2)
                        [lookup_time,lookup_dist]=mex_lookup_times8SP_and_dist(int32(0),int32(dimension_opt),int32(direction_opt),probe_coordsX,probe_coordsY,probe_coordsZ,pixel_coordsX,pixel_coordsY,pixel_coordsZ,vel_water,0.0,0.0,1.0/(exp_data.time(2)-exp_data.time(1)),int32(travels_calculated));
                    else
                        [lookup_time,lookup_dist]=mex_lookup_times8SP_and_dist(int32(0),int32(dimension_opt),int32(direction_opt),probe_coordsX,probe_coordsZ,pixel_coordsX,pixel_coordsZ,vel_water,single(0.0),single(0.0),sample_freq,int32(travels_calculated));
                    end
                case 2 %L
                    matMode=[1];
                    ...
                    if (dimension_opt>2)
                        [lookup_time,lookup_dist]=mex_lookup_times8SP_and_dist(int32(2),int32(dimension_opt),int32(direction_opt),probe_coordsX,probe_coordsY,probe_coordsZ,pixel_coordsX,pixel_coordsY,pixel_coordsZ,frontwall_coordX,frontwall_coordY,frontwall_coordZ,vel_water,vel_material_tx1,vel_material_tx2,sample_freq,int32(travels_calculated));
                    else
                        if (solver_opt == 1)
                            [lookup_time,lookup_dist]=mex_lookup_times8SP_and_dist(int32(1),int32(dimension_opt),int32(direction_opt),probe_coordsX,probe_coordsZ,pixel_coordsX,pixel_coordsZ,vel_water,vel_material_tx1,vel_material_tx2,sample_freq,int32(travels_calculated));
                        else
                            [lookup_time,lookup_dist]=mex_lookup_times8SP_and_dist(int32(2),int32(dimension_opt),int32(direction_opt),probe_coordsX,probe_coordsZ,pixel_coordsX,pixel_coordsZ,frontwall_coordX,frontwall_coordZ,vel_water,vel_material_tx1,vel_material_tx2,sample_freq,int32(travels_calculated));
                        end
                    end
                case 3 %T
                    matMode=[2];
                ...
                case 4 %LL
                    ...
                    if (dimension_opt>2)
                        [lookup_time,lookup_dist]=mex_lookup_times8SP_and_dist(int32(3),int32(dimension_opt),int32(direction_opt),probe_coordsX,probe_coordsY,probe_coordsZ,pixel_coordsX,pixel_coordsY,pixel_coordsZ,frontwall_coordX,frontwall_coordY,frontwall_coordZ,backwall_coordX,backwall_coordY,backwall_coordZ,vel_water,vel_material_tx1,vel_material_tx2,sample_freq,int32(travels_calculated));
                    else
                        if (solver_opt == 1)
                            [lookup_time,lookup_dist]=mex_lookup_times8SP_and_dist(int32(3),int32(dimension_opt),int32(direction_opt),probe_coordsX,probe_coordsZ,pixel_coordsX,pixel_coordsZ,frontwall_coordX,frontwall_coordZ,backwall_coordX,backwall_coordZ,vel_water,vel_material_tx1,vel_material_tx2,sample_freq,int32(travels_calculated));
                        else
                            [lookup_time,lookup_dist]=mex_lookup_times8SP_and_dist(int32(3),int32(dimension_opt),int32(direction_opt),probe_coordsX,probe_coordsZ,pixel_coordsX,pixel_coordsZ,frontwall_coordX,frontwall_coordZ,backwall_coordX,backwall_coordZ,vel_water,vel_material_tx1,vel_material_tx2,sample_freq,int32(travels_calculated));
                        end
                    end  
            ...
            focal_laws.pathDist(1:size(lookup_dist,1),:,:,i)=lookup_dist;
            focal_laws.matMode(1:length(matMode),i)=matMode;
        end
    otherwise
        ...
        for i=1:nRayPaths
            ii=rayPathsRequired3(i);
            rayPathsRequired4(ii)=i;
            disp('ERROR: Distance not coded for here');
            continue;
            switch (ii)
                case 1 %Contact
                    if (dimension_opt>2)
