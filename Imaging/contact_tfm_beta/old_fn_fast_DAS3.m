...
if use_gpu_if_available && (exist('gpuDeviceCount') == 2) && (gpuDeviceCount > 0)
    ...
    switch method
        case 'Same Tx and RX laws (nearest)'
            ...
            %lookup_ind= gpuArray(int32(focal_law.lookup_ind));
            %lookup_amp= gpuArray(single(focal_law.lookup_amp));
        ...
        case 'Different Tx and RX laws, FMC data (nearest)'
            ...
            focal_law.kern.GridSize = ceil(pixs./focal_law.kern.ThreadBlockSize(1));
            %lookup_ind_tx= gpuArray(int32(focal_law.lookup_ind_tx));
            %lookup_ind_rx= gpuArray(int32(focal_law.lookup_ind_rx));
            %lookup_amp_tx=gpuArray(single(focal_law.lookup_amp_tx));
            %lookup_amp_rx=gpuArray(single(focal_law.lookup_amp_rx));
        ...
        case 'Same Tx and RX laws (linear)'
            ...
            focal_law.kern.GridSize = ceil(pixs./focal_law.kern.ThreadBlockSize(1));
            %lookup_time= gpuArray(single(focal_law.lookup_time));
        ...
        case 'Different Tx and RX laws, FMC data (linear)'
            ...
            focal_law.kern.GridSize = ceil(pixs./focal_law.kern.ThreadBlockSize(1));
            %lookup_time_tx= gpuArray(single(focal_law.lookup_time_tx));
            %lookup_time_rx= gpuArray(single(focal_law.lookup_time_rx));
            time= gpuArray(single(exp_data.time));
            %lookup_amp_tx= gpuArray(single(focal_law.lookup_amp_tx));
            %lookup_amp_rx= gpuArray(single(focal_law.lookup_amp_rx));
