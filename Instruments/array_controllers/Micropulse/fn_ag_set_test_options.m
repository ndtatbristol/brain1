function [tx_no, rx_no] = fn_ag_set_test_options(test_options, varargin);
...
%clear focal laws
fn_ag_send_command('DXN 0 0', 0 , echo_on);
fn_ag_send_command('DLYS 0 0', 0, echo_on);
...
fn_ag_send_command('AWF 0 1', 0, echo_on);%specify full RF signal
fn_ag_send_command('AWFS 0 1', 0, echo_on);%specify full RF signal
...
pow2 = min([pow2, 10]);%don't know what max avs bvalue allowed is! to check
pow2 = max([pow2, 0]);
fn_ag_send_command(sprintf('AMP 0 13 %i', pow2), 0, echo_on);%avs and FMC mode
fn_ag_send_command(sprintf('AMPS 0 13 %i', pow2), 0, echo_on);%avs and FMC mode
actual_avs = 2 ^ pow2;
end
...
prf = max([prf, 1000]);
actual_prf = prf;
fn_ag_send_command(sprintf('PRF %i', actual_prf), 0, echo_on);%PRF
end

%sample frequency
function actual_sample_freq = fn_ag_set_sample_freq(sample_freq, echo_on)
reset_pause = 3;
[test_options, system_info] = fn_ag_get_test_options(echo_on);
if sample_freq == test_options.sample_freq
    actual_sample_freq = sample_freq;
    return;
end;
sample_freq = interp1([25, 50, 100] * 1e6, [25, 50, 100] * 1e6, sample_freq, 'nearest', 'extrap');
actual_sample_freq = sample_freq;
[res, err] = fn_ag_send_command(sprintf('RST %i', round(sample_freq / 1e6)), 10, echo_on);
pause(reset_pause);
end
...
fn_ag_send_command(sprintf('DOF %i', dof_val), 0, echo_on);%pulse amp
actual_bits = bits;
end
...
volt1 = min([volt1, 200]);
volt1 = max([volt1, 50]);
fn_ag_send_command(sprintf('PAV 1 128 %i', volt1), 0, echo_on);%pulse amp
...
fn_ag_send_command(sprintf('PAW 1 128 %i', width_ns), 0, echo_on);%pulse width
end
...
fn_ag_send_command(sprintf('GAT 0 %i %i', gate_start, gate_start+pts), 0, echo_on);%time points
fn_ag_send_command(sprintf('GATS 0 %i %i', gate_start, gate_start+pts), 0, echo_on);%time points
actual_pts = pts;
end
...
fn_ag_send_command(sprintf('FRQS 0 %i %i', filter_no, smoothing), 0, echo_on);
...
if size(tx_ch, 1) > 1
    fn_ag_send_command(sprintf('SWP 1 %i - %i',256 ,255 + size(tx_ch, 1)), 0, echo_on);
else
...
fn_ag_send_command(sprintf('GAN 0 %i', gain_val), 0, echo_on);%time points
fn_ag_send_command(sprintf('GANS 0 %i', gain_val), 0, echo_on);%time points
