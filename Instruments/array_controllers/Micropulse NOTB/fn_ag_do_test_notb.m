function time_data = fn_ag_do_test_notb(varargin)
...
[raw_result, err_msg] = fn_ag_send_command_notb('CALS 0', timeout, echo_on,buffersize);
...
switch dof
    ...
    otherwise
        ...
        disp(['Bytes per signal ',num2str(bytes_per_signal)])
        disp(['raw data length ',num2str(length(raw_result(:)))])
...
if rem(length(raw_result), bytes_per_signal) ~= 0
    disp(['Bytes per signal ',num2str(bytes_per_signal)])
    disp(['raw data length ',num2str(length(raw_result))])
    disp(['Data size mismatch ',num2str(rem(length(raw_result), bytes_per_signal))])
    error('Incomplete data');
    return;
end;
