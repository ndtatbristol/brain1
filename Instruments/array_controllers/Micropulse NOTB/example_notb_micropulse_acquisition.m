...
if exist('tcpip_obj') && isa(tcpip_obj, 'tcpclient')
    clear tcpip_obj;
    disp('Clearing');
end
% if exist('tcpip_obj') && isa(tcpip_obj, 'tcpipclient');
%     fclose(tcpip_obj);
%     delete(tcpip_obj);
%     clear('tcpip_obj');
%     disp('Clearing');
...
%make tcp/ip object global. Dirty fix, will be improved with objectf
%orientated version.
...
%Connect
if ~fn_ag_connect_notb(ip_address, port_no, 1)
...
%Reset if required
if full_reset
    reset_result = fn_ag_reset_notb(echo_on);
end;
...
[exp_data.tx, exp_data.rx] = fn_ag_set_test_options_notb(test_options, echo_on);

pause(0.2)
%Capture one set of data from Micropulse
tic;
exp_data.time_data = fn_ag_do_test_notb(echo_on);
time_to_capture = toc;
