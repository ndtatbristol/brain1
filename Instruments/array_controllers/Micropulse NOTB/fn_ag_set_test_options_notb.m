function [tx_no, rx_no] = fn_ag_set_test_options_notb(test_options, varargin);
...
%clear focal laws
fn_ag_send_command_notb('DXN 0 0', 0 , echo_on);
fn_ag_send_command_notb('DLYS 0 0', 0, echo_on);
...
fn_ag_send_command_notb('AWF 0 1', 0, echo_on);%specify full RF signal
fn_ag_send_command_notb('AWFS 0 1', 0, echo_on);%specify full RF signal
...
fn_ag_send_command_notb(sprintf('BUFF %i', test_options.buff_mode), 0, echo_on);

if test_options.sample_bits == 8
    test_options.bytes_per_point = 1;
end
    
buffersize = (test_options.header_length+(test_options.time_pts.* test_options.bytes_per_point)) .* length(tx_no)  + 2 + 2.*test_options.buff_mode; %the plus to is to account for the [1 1] at the end of a message
...
pow2 = max([pow2, 0]);
fn_ag_send_command_notb(sprintf('AMP 0 13 %i', pow2), 0, echo_on);%avs and FMC mode
fn_ag_send_command_notb(sprintf('AMPS 0 13 %i', pow2), 0, echo_on);%avs and FMC mode
actual_avs = 2 ^ pow2;
end
...
prf = max([prf, 1]);
actual_prf = prf;
fn_ag_send_command_notb(sprintf('PRF %i', actual_prf), 0, echo_on);%PRF
end

%sample frequency
function actual_sample_freq = fn_ag_set_sample_freq(sample_freq, echo_on)
reset_pause = 3;
[test_options, system_info] = fn_ag_get_test_options_notb(echo_on);
...
actual_sample_freq = sample_freq;
[res, err] = fn_ag_send_command_notb(sprintf('RST %i', round(sample_freq / 1e6)), 10, echo_on);
pause(reset_pause);
end
...
fn_ag_send_command_notb(sprintf('DOF %i', dof_val), 0, echo_on);%pulse amp
actual_bits = bits;
end
...
volt1 = min([volt1, 200]);
volt1 = max([volt1, 50]);
fn_ag_send_command_notb(sprintf('PAV 1 128 %i', volt1), 0, echo_on);%pulse amp
...
fn_ag_send_command_notb(sprintf('PAW 1 128 %i', width_ns), 0, echo_on);%pulse width
end
...
fn_ag_send_command_notb(sprintf('GAT 0 %i %i', gate_start, gate_start+pts), 0, echo_on);%time points
fn_ag_send_command_notb(sprintf('GATS 0 %i %i', gate_start, gate_start+pts), 0, echo_on);%time points
actual_pts = pts;
end
...
fn_ag_send_command_notb(sprintf('FRQS 0 %i %i', filter_no, smoothing), 0, echo_on);
...
for fl_ii = 1:transmit_laws %loop through focal laws
    ...
    for tx_ii = 1:use_els %loop through focal laws
        fn_ag_send_command_notb(sprintf('TXF %i %i -1', fl_ii, tx_ii), 0, echo_on);%law, ch, del
    end;
    ...
    for tx_ii = 1:length(tx_nos) %add each transmitter specified for focal law
        fn_ag_send_command_notb(sprintf('TXF %i %i %i', fl_ii, tx_nos(tx_ii), tx_delay_law_array(fl_ii,tx_nos(tx_ii))), 0, echo_on);%law, ch, del
    end;
    %clear existing rx delays
    fn_ag_send_command_notb(sprintf('RXF %i 0 -1 0', fl_ii), 0, echo_on);%law, ch, del
    rx_nos = find(rx_ch(fl_ii,:));
    for rx_ii = 1:length(rx_nos); %add receivers to all focal laws
        counter = counter + 1;
        fn_ag_send_command_notb(sprintf('RXF %i %i %i 0', fl_ii, rx_nos(rx_ii), rx_delay_law_array(fl_ii,rx_nos(rx_ii))), 0, echo_on);%law, ch, del, trim_amp
    ...
    fn_ag_send_command_notb(sprintf('TXN %i %i', 255 + fl_ii, fl_ii), 0, echo_on);
    fn_ag_send_command_notb(sprintf('RXN %i %i', 255 + fl_ii, fl_ii), 0, echo_on);
end;

%assign tests to sweep 1
if size(tx_ch, 1) > 1
    fn_ag_send_command_notb(sprintf('SWP 1 %i - %i',256 ,255 + size(tx_ch, 1)), 0, echo_on);
else
    fn_ag_send_command_notb(sprintf('SWP 1 %i',256), 0, echo_on);
end;
%disable all sweeps
fn_ag_send_command_notb('DIS 0', 0, echo_on);
fn_ag_send_command_notb('DISS 0', 0, echo_on);

%enable test 256
fn_ag_send_command_notb('ENA 256', 0, echo_on);
%enable sweep 1
fn_ag_send_command_notb('ENAS 1', 0, echo_on);
end
...
fn_ag_send_command_notb(sprintf('GAN 0 %i', gain_val), 0, echo_on);%time points
fn_ag_send_command_notb(sprintf('GANS 0 %i', gain_val), 0, echo_on);%time points
