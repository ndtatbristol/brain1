function [info, h_fn_connect, h_fn_disconnect, h_fn_reset, h_fn_acquire, h_fn_send_options] = fn_notb_micropulse_wrapper(dummy)
info.name = 'Peak NDT Micropulse no toolbox';
echo_on = 0;
...
big_inv_s = [];
    ...
    function exp_data = fn_acquire(dummy)
        ...
        exp_data.time_data = fn_ag_do_test_notb;
        if hadamard_excite
    ...
    function fn_send_options(options, array, material)
        ...
        [tx_no, rx_no] = fn_ag_set_test_options_notb(options, echo_on);
        if hadamard_excite
    ...
    function fn_reset(dummy)
        fn_ag_reset_notb(echo_on);
    end

    function res = fn_disconnect(dummy)
        fn_ag_disconnect_notb(echo_on);
        connected = 0;
        res = connected;
    end

    function res = fn_connect(options)
        connected = fn_ag_connect_notb(options.ip_address, options.port_no, echo_on);
        res = connected;
    end
