...
%
info.options_info.aos_driver_path.label = 'AOS driver path';
info.options_info.aos_driver_path.default = 'C:\Program Files\AOS\OEMPA 1.2.0.1';
...
info.options_info.sample_freq.constraint = {'10', '25', '50'};
...
%         if strcmp(mxGetAcquisitionAscanBitSize(deviceId,1),'8Bits')
%             Nbit = 8;
%         elseif strcmp(mxGetAcquisitionAscanBitSize(deviceId,1),'12Bits')
%             Nbit = 12;
%         elseif strcmp(mxGetAcquisitionAscanBitSize(deviceId,1),'16Bits')
%             Nbit = 16;
%         end
%         if Nbit>=12
...
%         end
        ...
        %-------------------------------------------------------------------------%
        Nrow = 0; Ncol = 0;
        while (Nrow~=PointCount) | (Ncol~=CycleCount)
            [AscanCount, fifoAscanLost1, total] = mxGetAcquisitionAscanFifoStatus(deviceId) ;
            [lastDisplay, displayOn] = TimerDisplay(tstartTimerDisplay,lastDisplay);
            if displayOn
        ...
        % Disable pulser if this was last frame or only one was required
        if ~instr_control_data.play_button_state || instr_control_data.play_once
