function [focal_law, surface] = fn_calc_immersion_tfm_focal_law3(exp_data, mesh, orig_surface, immersion_options, varargin)
if nargin < 5
...
zi_12 = max([find(mesh.z(:,1) < min(surface.z)); 1]); %last index of couplant-only region
...
focal_law.lookup_time = [zeros(zi_12 - 1, size(lookup_time3, 2), size(lookup_time3, 3)); lookup_time2; lookup_time3];
focal_law.lookup_amp = [zeros(zi_12 - 1, size(lookup_time3, 2), size(lookup_time3, 3)); lookup_amp2; lookup_amp3];
...
%matrix of element positions and angles
e = [array.el_xc(:), array.el_zc(:)];
e_theta = atan2(-array.el_xc(:) + array.el_x1(:), array.el_zc(:) - array.el_z1(:));

%matrix of times and angles from every element to every surface point
couplant_time = zeros(length(array.el_xc), size(s,2));
couplant_angs = zeros(length(array.el_xc), size(s,2));
for ii = 1:size(s,1)
    couplant_time = couplant_time + (repmat(e(:,ii), [1, size(s, 2)]) - repmat(s(ii,:), [size(e, 1), 1])) .^ 2;
end
couplant_time = sqrt(couplant_time) / immersion_options.couplant_velocity;
couplant_angs = atan2(repmat(s(2,:), [size(e, 1), 1]) - repmat(e(:,2), [1, size(s, 2)]), ...
    repmat(s(1,:), [size(e, 1), 1]) - repmat(e(:,1), [1, size(s, 2)])) - repmat(e_theta, [1, size(s,2)]);
...
%     keyboard
    angle_in_couplant = couplant_angs(ii, jj);
    amp = amp.' .* double(((immersion_options.max_angle_in_couplant > 0) & (abs(angle_in_couplant) < immersion_options.max_angle_in_couplant)) | (~immersion_options.max_angle_in_couplant .* ones(size(angle_in_couplant))));
    if use_coarse_mesh
